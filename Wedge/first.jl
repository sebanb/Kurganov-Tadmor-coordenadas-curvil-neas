...
if t0 == 0
    ...
    for i = 1:Dom[end, 1]
        ...
        for j = 1:n
            for k = 1:m
                u1[j, k], v1[j, k] =
                    Car2Cur(u_xini, u_yini, ξ[j], η[k], Dom[i, 2], i)
                E1[j, k] =
                    ρ1[j, k] * (
                        0.5 * (u1[j, k]^2 + v1[j, k]^2) +
                        p1[j, k] / (ρ1[j, k] * (γ - 1))
                    )
            end
        end
    ...
    u = load(name * string(0.06) * ".jld")["u_ξ"]
    v = load(name * string(0.06) * ".jld")["u_η"]
    p = load(name * string(0.06) * ".jld")["p"]
    E = load(name * string(0.06) *".jld")["E"]
    ρ = load(name* string(0.06) *".jld")["ρ"]
    t = load(name * string(0.06) * ".jld")["t"]
    T = load(name * string(0.06) * ".jld")["T"]
end
...
while t < tf
    ...
    for i = 1:Dom[end, 1]
        ...
        sol = solve(prob, RK4(), dt = Δt, save_start = false, adaptive = false)
        ρdom = sol.u[1].ρ
...
for i = 1:Dom[end, 1]
    ...
    for j = 1:n
        for k = 1:m
            if i == 3
                x1[j, k] = 1 - changcoor(ξ[j], η[m-k+1], Dom[i, 2])[2]
                y1[j, k] = changcoor(ξ[j], η[m-k+1], Dom[i, 2])[1] + 1 * tand(15)
            else
            x1[j, k], y1[j, k] = changcoor(ξ[j], η[k], Dom[i, 2])
        end
            u1[j, k], v1[j, k] =
                Cur2Car(u[i][j, k], v[i][j, k], ξ[j], η[k], Dom[i, 2])
            U_1[j, k] = sqrt(u1[j, k]^2 + v1[j, k]^2)
        end
    end
    push!(x, x1)
    push!(y, y1)
...
saved_files = vtk_multiblock("full_domainaadsa_" * name * string(tf)) do vtm
    for i = 1:Dom[end, 1]
